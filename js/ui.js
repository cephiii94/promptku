...
// === 3. RENDER FUNCTIONS ===
export const populateCategoryOptions = (categories, activeCategory, onChipClick) => {
    // Dropdown Form Upload
    if (els.promptCategoryInput) {
        els.promptCategoryInput.innerHTML = '<option value="" disabled selected>-- Pilih Kategori --</option>';
    ...
    if (els.homeCategoryList) {
        ...
        categories.forEach(category => {
            ...
            chip.className = `category-chip ${activeCategory === category ? 'active' : ''}`;
...
export const renderPrompts = (prompts, currentUser, currentPage, itemsPerPage) => {
    if (!els.promptGrid) return;

    if (!prompts.length) {
         els.promptGrid.innerHTML = '<p style="text-align:center; width:100%; padding: 2rem; color: #666;">Tidak ada prompt yang ditemukan.</p>';
        return;
    }
...
export const renderPagination = (totalPages, currentPage, onPageChange) => {
    ...
    if (startPage > 1) {
        ...
        if (startPage > 2) html += `<span style="padding:0 5px; color:#aaa;">...</span>`;
    }
    ...
    if (endPage < totalPages) {
        if (endPage < totalPages - 1) html += `<span style="padding:0 5px; color:#aaa;">...</span>`;
    ...
    els.paginationContainer.querySelector('.prev-btn')?.addEventListener('click', () => {
        if(currentPage > 1) onPageChange(currentPage - 1);
    });
    els.paginationContainer.querySelector('.next-btn')?.addEventListener('click', () => {
...
export const fillPromptModal = (data, isAdmin) => {
    ...
    if(els.promptCategoryInput) els.promptCategoryInput.value = data?.category || ''; 
    ...
    if (els.mayarLinkContainer) {
        if (isPrem) {
            ...
            if(els.promptTextInput) els.promptTextInput.placeholder = "Tulis deskripsi singkat / teaser di sini. JANGAN TULIS PROMPT ASLI!";
        } else {
...
// === 6. FULL VIEW MODAL ===
export const showFullViewModal = (data, currentUser, currentViewIndex, totalItems) => {
    ...
    // Modal Nav Logic
    if(els.modalNavPrev) els.modalNavPrev.style.display = (currentViewIndex === 0) ? 'none' : 'flex';
    if(els.modalNavNext) els.modalNavNext.style.display = (currentViewIndex === totalItems - 1) ? 'none' : 'flex';
